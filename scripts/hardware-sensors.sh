...
check_ipmitool() {
    [[ -r /usr/bin/ipmitool ]] || error_exit "ipmitool not accessible. Check installation and permissions."
}
validate_args() {
    if [[ $# -gt 1 ]]; then
        error_exit "Usage: $0 [--brief]"
    fi
}
# Main
check_ipmitool
...
if safe_exec "sudo /usr/bin/ipmitool sdr type temperature"; then
    sudo /usr/bin/ipmitool sdr type temperature 2>/dev/null | grep -v "Not Present" | head -15 | while read -r line; do
...
if safe_exec "sudo /usr/bin/ipmitool sdr type fan"; then
    sudo /usr/bin/ipmitool sdr type fan 2>/dev/null | grep -v "Not Present" | head -12 | while read -r line; do
        if [[ "$line" == *"RPM"* ]]; then
